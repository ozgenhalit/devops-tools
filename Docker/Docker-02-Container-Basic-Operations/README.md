...
- Run the second `ubuntu` os with interactive shell open and name container as `stevie` and show that this `ubuntu` container is different from the previous one.

```bash
docker run -i -t --name stevie ubuntu
```
...
- Restart the `stevie` container by its name and list only running containers.

```bash
docker start stevie && docker ps
```

- Connect to the interactive shell of running `stevie` container and `exit` afterwards.

```bash
docker attach stevie
```

- Show that `stevie` container has stopped by listing all containers.
...
- Show that we can get more information about `stevie` container by using `docker inspect` command and explain the properties.

```bash
docker inspect stevie | less
```
